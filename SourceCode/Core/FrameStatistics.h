...
{
    // Collects frame durations for the performance window. Two kinds of data, for two different uses:
    //
    // 1. Numbers to read (average frame time, FPS, the longest frame) are calculated over intervals of ReportInterval
    //    seconds and change only when an interval ends. Numbers that changed every frame would be unreadable at high
    //    frame rates: at 1700 FPS the last digits flicker hundreds of times per second.
    //
    // 2. The last MaxSampleCount frame durations, for the frame time graph, where every frame matters. They are stored
    //    in a ring buffer: a fixed array where every new frame overwrites the oldest one. Once the array is full,
